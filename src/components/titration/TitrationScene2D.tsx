...
const TILE_Y = 540;
const PIPET_BASE_TRANSFORM = "translate(72, 438) rotate(-28) scale(0.92)";
...
export const TitrationScene2D: React.FC<TitrationScene2DProps> = ({
  ...
  const rinsingBuret = procedureAnimation === "rinsedBurette";
  const rinsingPipet = procedureAnimation === "rinsedPipette";
  const pipettingSample = procedureAnimation === "pipettedSample";
  const addingIndicator = procedureAnimation === "addedIndicator";
  const fillingBuret = procedureAnimation === "filledBurette";
  const readingBurette = procedureAnimation === "readStart" || procedureAnimation === "readEnd";
  const pipetTransform = PIPET_BASE_TRANSFORM;

  return (
    <div className={`titration-scene-wrapper ${procedureAnimation ? `titration-scene-${procedureAnimation}` : ""}`} aria-label="Titratielab 2D scene">
      <svg
        viewBox="0 0 420 640"
        width="100%"
        height="100%"
      ...
      >
        ...
        {/* ── Buret ── */}
        {burette && (
          <g className={`titration-buret ${rinsingBuret ? "titration-buret-rinse" : fillingBuret ? "titration-buret-fill" : ""}`}>
            {/* Outer glass tube */}
            <rect
              x={BURET_X - 8}
              y={BURET_TOP_Y}
            ...
            <rect
              ...
              opacity="0.45"
            />
            {rinsingBuret && <rect x={BURET_X - 3} y={BURET_TOP_Y + 14} width={6} height={BURET_HEIGHT_PX - 28} rx="3" fill="rgba(126, 200, 227, 0.20)" />}
          </g>
        )}
        ...
        {/* ── Erlenmeyer flask ── */}
        {flask && (
          <g className={`titration-flask ${pipettingSample ? "titration-flask-sample" : addingIndicator ? "titration-flask-indicator" : ""}`}>
            ...
            <path
              ...
              opacity="0.4"
            />
            {(pipettingSample || addingIndicator) && (
              <ellipse cx={FLASK_CX} cy={536} rx={52} ry={18} fill={pipettingSample ? "rgba(31,75,153,0.08)" : "rgba(200,88,143,0.12)"} opacity="0.9" />
            )}
          </g>
        )}
        ...
        {pipet && (
          <g className={`titration-pipet ${rinsingPipet ? "titration-pipet-rinse" : pipettingSample ? "titration-pipet-sample" : ""}`} transform={pipetTransform} opacity="0.88">
            <g>
              <rect x="-3" y="-80" width="6" height="100" rx="3" fill="url(#glassGrad)" stroke="#8ab4cc" strokeWidth="1" opacity={pipettingSample ? "0.96" : "1"} />
        ...
        {burette && (
          <g className={`titration-reading-indicator ${readingBurette ? "titration-reading-indicator-active" : ""}`}>
            <line
              x1={BURET_X + 8}
              y1={liquidTopY}
              x2={BURET_X + 50}
