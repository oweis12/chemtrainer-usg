...
export function ConceptDiagram({ kind }: { kind: ConceptDiagramId }) {
  ...
  return <figure className={`concept-diagram concept-diagram-${kind}`}><div className="diagram-heading"><Sparkle size={16} weight="fill" /> <span>{meta.title}</span></div><DiagramBody kind={kind} /><figcaption>{meta.caption}</figcaption></figure>;
}
...
function DiagramBody({ kind }: { kind: ConceptDiagramId }) {
  ...
  if (kind === "polarity") return <div className="diagram-polarity diagram-native-svg"><BondPolarityDiagram variant="polarCovalent" /></div>;
